  ...
  <modals-modal v-model="show" :width="400" height="100%">
    <template #outer>
      <div class="absolute top-11 left-4 z-40">
        <p class="text-fg text-2xl truncate">{{ $strings.LabelYourBookmarks }}</p>
      </div>
    </template>
      ...
      <div class="w-full rounded-lg bg-primary border border-border overflow-y-auto overflow-x-hidden relative mt-16" style="max-height: 80vh" @click.stop.prevent>
        <div class="w-full h-full p-4" v-if="showBookmarkTitleInput">
          <div class="flex mb-4 items-center">
            <div class="w-9 h-9 flex items-center justify-center rounded-full hover:bg-fg hover:bg-opacity-10 cursor-pointer" @click.stop="showBookmarkTitleInput = false">
        ...
        <div v-if="canCreateBookmark && !showBookmarkTitleInput" class="flex px-4 py-2 items-center text-center justify-between border-b border-fg/10 bg-success cursor-pointer text-fg text-opacity-80 sticky bottom-0 left-0 w-full" @click.stop="createBookmark">
          <ui-ph-icon name="add" :size="22" />
