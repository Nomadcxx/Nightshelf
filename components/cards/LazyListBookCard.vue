  ...
  >
    ...
    <div class="h-full flex relative overflow-hidden rounded-xl bg-secondary/60">
      <div class="list-card-cover relative overflow-hidden rounded-l-xl">
        ...
        <div v-if="!isPodcast && !collapsedSeries" class="absolute bottom-0 left-0 h-1 shadow-sm max-w-full z-10 rounded-b" :class="itemIsFinished ? 'bg-success' : 'bg-warning'" :style="{ width: coverWidth * userProgressPercent + 'px' }"></div>
      </div>
      ...
      <div v-if="showPlayButton" class="absolute top-0 bottom-0 right-0 h-full flex items-center justify-center z-20 pr-1">
        <button type="button" class="relative rounded-full bg-fg-muted/50" :class="{ 'p-2': !playerIsStartingForThisMedia }" @click.stop.prevent="play">
          <span v-if="!playerIsStartingForThisMedia" class="material-symbols text-2xl fill text-fg">{{ playerIsPlaying ? 'pause' : 'play_arrow' }}</span>
